...
This vulnerability is located in the `cstecgi.cgi` file within the `FUN_00422928` function, with the alias `setTracerouteCfg`.


![](Pasted%20image%2020231023195516.png)


`cstecgi.cgi` processes different function interfaces based on the value of `topicurl`. While it validates the legality of incoming commands, filtering out command injection vulnerabilities, it does not impose length restrictions on the incoming parameter `command`. The `sprintf` function is used to format and concatenate the parameter `command` into the `acStack_90` variable, which results in a stack overflow vulnerability.
